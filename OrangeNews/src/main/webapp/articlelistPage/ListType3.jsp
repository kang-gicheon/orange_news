...
<c:set var="articlesList" value="${articlesMap.articleType3 }" />
...
<body id="body">
	...
	<section>
		<div class="type-section">
			<div class="type-section-title">사 회 / 문 화</div>
			<hr>
			<c:choose>
								...
								<c:forEach var="page" begin="1" end="10" step="1">
									<c:if test="${section > 1 && page==1 }">
										...
										<li class="page-item"><a class="page-link"
											href="${contextPath }/news/type3List.do?section=${section-1}&pageNum=${section*10-1}">Pre</a></li>

									</c:if>
									<a class="no-uline"
										href="${contextPath }/news/type1List.do?section=${section}&pageNum=${page}">
										${(section-1)*10 + page} </a>
									<c:if test="${page==10 }">
										<li class="page-item"><a class="page-link" href="#">Next</a></li>
										<a class="no-uline"
											href="${contextPath }/news/type3List.do?section=${section+1}&pageNum=${section*10+1}">&nbsp;next</a>
									</c:if>
								</c:forEach>
							</c:when>
							<c:when test="${totArticles==100 }">
								...
								<c:forEach var="page" begin="1" end="${totArticles/10+1 }"
									...
									<c:choose>
										<c:when test="${page==pageNum }">
											<li class="page-item"><a class="page-link"
												href="${contextPath }/news/type3List.do?section=${section}&pageNum=${page}">${page }</a></li>
										</c:when>
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="${contextPath }/news/type3List.do?section=${section}&pageNum=${page}">${page }</a></li>
