...
public class ArticleService {
	...
	public ArticleService() { //실행되면 DAO를 만든다.
		articleDAO = new ArticleDAO();
	}
	
	public void addArticle (ArticleVO article) { //기사를 작성(추가)
		articleDAO.insertNewArticle(article);
	}
	
	public ArticleVO updateAction(ArticleVO article) { //반응을 업데이트, 그 결과 article로 반환
		articleDAO.updateReact(article);
		return article;
	}
	
	public ArticleVO updateReccount(ArticleVO article) { //추천을 업데이트, 그 결과 article로 반환
...
//		articlesMap.put("totArticles", totArticles);	//a안 - 기사 글 갯수에 따라 페이징 넘버 생성
	...
	public void viewArticle(ArticleVO article) { //기사 조회
